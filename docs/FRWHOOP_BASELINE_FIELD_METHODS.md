Longitudinal baseline — field methods (WHOOP 5.0)
...
The primary baseline, for this version, is the last 7 days (EWMA). A second, slower copy looks back 60 days but skips those same 7 days (median). The two copies are never averaged together. That pair is the v1 engine. It is not the final engine: the intended successor is a robust state-space model of level, trend, and uncertainty (section 1.9).

Two modes share that math. Before a recorded start, one adaptive track answers what is usual now. After a qualifying start, freeze Usual before treatment from days strictly before that clock time, and open a new adaptive epoch so Usual since treatment only sees post-start nights. Patient and caregiver see the same named list (condensed plan section 1.8). The two long copies are never averaged.

Layer 1 is one series at a time. Each snapshot also stores the fast–slow gap, a confidence tag, persistence counts, and a regime flag so a later watchdog can read them. This pass does not fire alerts and does not mix metrics into one score.

Companion writeup with the same contract, build steps, and worked numbers: docs/FRWHOOP_BASELINE_CONDENSED_PLAN.md. Source evaluation: docs/FRWHOOP_BASELINE_FINAL_PLAN.md.
...
When the longer copy is established (n_long ≥ 14) and spread_long exists:

z_long(d) = (x(d) − center_long) / spread_long    for each quality-OK d in [T−7, T−1]

Student-t weight (v1 default, ν = 4):
...
For each new candidate long day d, residual e(d) = x(d) − last live center_long, scaled by spread_long. Robust one-sided CUSUM (both directions, take the max):

S_t = max( 0, S_{t−1} + |e(d)| / spread_long − k_cusum )

k_cusum = 0.5 (v1). Missing days do not increment S and do not reset it.

p_change = 1 − exp( −S_t / h_cusum )    h_cusum = 20 (v1)

Store S_t and p_change every day. regime_shift = 1 if p_change ≥ p_thr (v1 p_thr = 0.90). Then hold the last established center_long and spread_long. Keep the old snapshots.
...
A new epoch is allowed when the person (or a later rule) accepts the new state; this pass only stores p_change and holds. If the new state is a recorded intervention, also open trial mode (section 1.8). Do not use p_change as a substitute for an explicit start timestamp.

Seven consecutive fever days at |z| ≈ 4.2 give S ≈ 26 and p_change ≈ 0.73 (< 0.90): trim them, do not declare a new usual. Fourteen such days give S ≈ 52 and p_change ≈ 0.93: hold. That is a probability, not a 14-day cliff by fiat; k_cusum, h_cusum, and p_thr are tunable.

1.7 Confidence, parameters, persistence (no alerts)

Confidence (stored; not an on-screen lecture):

show_7: n_7 ≥ 4
show_long: n_long ≥ 4
established_long: n_long ≥ 14
conf_spread: high only if n_learn = 7, or n_learn ≥ 4 and established_long supplied the borrow
alert_eligible: established_long and not stale and not (regime_shift with no new epoch). Still not an alert.
...
Layer 1 does not combine series. Layer 2 (later) may require several series at once (example: RHR up, HRV down, respiration up, wrist temperature up). Do not average those z-scores into one 0–100 number here.

Parameters (v1 defaults; stored on the snapshot as param_set = v1). They are starting values to validate on real WHOOP 5.0 days, not proven optima:
...
n_7_show = 4
n_long_show = 4
n_long_established = 14
n_long_waking = 21   # waking_load, awake_active, and continuous (activity mix); sleep and awake_rest use 14
n_borrow = 7
p_regime = 0.35
persist_window = 3
persist_hits = 2
wash_in_days = 7
washout_days = 7
floors = the metric list in section 2

Changing a parameter writes a new version string. It does not edit old snapshots.

1.8 Names, and trial/intervention mode

Patient and caregiver see the same named list. Engine fields stay in the log.

Off when-well, Off this week, Week vs when-well, Days in a row off, Has usual changed?, Usual before treatment, Usual since treatment, Change since start, Change vs old usual, Bigger than sensor noise?, Sensor noise / body variation, Night-to-night hangover / effective nights, Settling in / on treatment / washing out, Why missing, Other things going on, What device and math / Device or math changed.

After a qualifying start: freeze Usual before treatment from days strictly before the stored clock time, and open a new adaptive epoch so Usual since treatment only sees post-start nights. Dose change, interruption, restart, and stop relabel phase only. Full gates, MDC, r1, missingness, and provenance are numbered 1–6 in docs/FRWHOOP_BASELINE_CONDENSED_PLAN.md section 1.8. Do not freeze thin data. Missing is never 0. A device or math change after start is not Change since start.

expected(T) = center_long_frozen + slope_long × (T − T_freeze)
z_trial_traj = (today − expected(T)) / spread_long_frozen

Primary contrast is Change since start vs Usual before treatment, on days with Other things going on = none and Why missing = none.

1.9 Intended engine: robust state-space

v1 returns the 7-day EWMA and the gapped 60-day median. That pair is a strong first snapshot. It is not the last baseline engine.
...
Until a state-space version string is fit on real WHOOP 5.0 days, keep returning v1. New snapshots from that model do not edit old EWMA/median rows.
...
For every series we keep two snapshots from the same daily numbers. They are not averaged together. Compute the longer median snapshot first (with trim), then the 7-day EWMA snapshot (with Student-t downweight). That pair is the adaptive track (normal mode). After a qualifying intervention, a frozen copy of that pair is the trial track. Post-treatment days update only the adaptive track.

Each snapshot has: center, spread, expected band, signed delta, z-score, coverage, context, valid-day count, last update, version, plus gap, gap_z, n_learn, confidence flags, persistence fields, regime_shift, and param_set.

7-day (primary):

α = 2 / (span_7 + 1)   (v1: 0.25)
...
k = k_band   (v1: 2)
band = [center − k × spread, center + k × spread]
delta = today − center
z = (today − center) / spread

Also:

gap = center_7_raw − center_long
z_long(T) = (today − center_long) / spread_long

Trial track (only after a qualifying freeze at t0; same numbers as above, copied and frozen):

expected(T) = center_long_frozen + slope_long × (T − T_freeze)
delta_trial_traj = today − expected(T)
z_trial_traj = delta_trial_traj / spread_long_frozen
delta_trial_level = today − center_long_frozen
...
confounder on day T: none, illness, hospitalization, travel, sleep_disruption, exercise_change, concomitant_med (one or more flags)
Primary trial contrast uses days with confounder = none and missing_cause = none. Other days stay in the log.
...
k = k_band (v1: 2). Band is center ± k × spread (~95% under a normal model if spread ≈ σ and k = 2). Pulse’s ±1.65 and Cadence’s |z| ≥ 1.5 are different cuts. Validate k on real days; do not treat 2 as optimal. The 7-day band is the primary range (this week, after freeze). The longer band is the secondary when-well range.

z-score. z = (today − center) / spread. Primary z is versus the 7-day EWMA; secondary z is versus the longer median. For HRV, subtract and divide on ln(RMSSD); convert delta and band to ms with exp. SpO₂ delta is in percentage points. Steps and active minutes stay in steps and minutes.
...
Floors (minimum spread so a flat week does not make z explode). v1 starting values; validate on real days:

sleep resting HR: 2 bpm
awake-rest HR: 3 bpm
awake-active HR: 4 bpm
continuous HR: 5 bpm
ln(RMSSD): 0.08
respiration: 0.5 breaths/min
wrist temperature: 0.3 °C
SpO₂ (mean or nadir): 0.5 percentage points
daily steps: 500 steps
daily active minutes: 10 min

7-day status: omit unless n_7 ≥ 4 (provisional show). Longer copy: n_long < 4 calibrating; 4–13 provisional; ≥ 14 established. conf_spread and alert_eligible follow section 1.7. Stale if no quality-OK day for > 14 calendar days. A later watchdog may page only if alert_eligible and persistence fields agree; it still must not page on a lone z.
...
Every civil day also stores: missing_cause (none if a value exists), confounder flags, treatment phase (none before t0), provenance pointer. Missingness is kept; it is not dropped without a cause code.
...
4. Fields (each exists on the 7-day EWMA copy first, then the longer median copy)

4.1 Center

Longer center (secondary), computed first: median of the trimmed list in [T−60, T−8], or the held established value if regime_shift. This week has weight 0.

7-day center (primary): span-7 EWMA, α = 2/(span_7+1), raw weight α × (1−α)^age with age 0 at T−1, times λ(d) (section 1.4). Missing days get raw weight 0 and are not filled with zero. Renormalize so remaining learn weights sum to 1. Need n_learn ≥ 4 to update; else hold. Always store center_7_raw from quality-OK days with λ ignored.

Need at least four numbers in that window or that copy of center is omitted (show gate). alert_eligible needs the longer copy established.

NightSignal / Alavi 2022 use a rest-gated overnight median for the longer copy. Gadaleta 2021 leaves the last 7 days out of that longer usual; those 7 days are the primary EWMA. Vela’s recovery card is a 21–42 day median (ungapped) — not the 7-day EWMA. Oura HRV Balance is a short window versus a long window. HRV-Foundation-Forecasting treats EWMA as a classical short-horizon tracker.

4.2 Spread

On the days that trained that copy’s center: MAD of |x − that center|, then × 1.4826, then not smaller than the metric floor. For the 7-day copy the MAD is around the EWMA on learn days, then mixed with spread_long when n_learn < n_borrow (section 1.5).

7-day spread is this week’s scatter after freeze and borrow (primary). Longer spread is scatter when well, after trim (secondary). Do not average the two centers. Borrowing spread is a scale mix only.

Leys 2013 and NIST give MAD × 1.4826. Vela uses the same robust SD on their median center. Four-point MAD is still jumpy; that is why we borrow.

4.3 Expected band

[center − k_band × spread, center + k_band × spread]. v1 k_band = 2. For HRV, convert the bounds with exp so the displayed range is in milliseconds. SpO₂ stays in percentage points. Steps stay in steps. Active minutes stay in minutes.

7-day band is the primary range (this week, around the downweighted EWMA). Longer band is the secondary when-well range (around the median). With full-weight training, a fever week can sit inside the 7-day band and outside the longer band at the same time. With Student-t downweight, gap_z still records the chase that was avoided.

TemPredict (Mason 2022) is a person-specific nightly range from a personal center and scale. Pulse uses about ±1.65 SD on a 30-day mean; that is not this band. Cadence |z| ≥ 1.5 is a later flag, not the usual range.

4.4 Signed delta and z-score

delta = today − that copy’s center, in display units (bpm, °C, ms, percentage points, steps, minutes).
z = (today − center) / spread. For HRV, today and center are on ln(RMSSD) before this division; display delta in milliseconds. SpO₂ stays in percentage points. Steps stay in steps. Active minutes stay in minutes.

Primary: 7-day delta and z versus the frozen EWMA (can still be large at the end of a fever week).
Secondary: longer delta and z versus the median when well. Always store gap = center_7_raw − longer median, and gap_z.

Vela: Z_HRV = (ln(HRV) − median(ln(HRV))) / MAD_robust on their long history. We store z versus the 7-day EWMA and versus the longer median. It is not passed through a 0–100 mix. Mishra 2020 is resting HR versus the person’s own baseline.

4.5 Quality and coverage

Day-level gate (same for both copies): enough in-bed samples; or at least 30 still waking minutes (awake-rest HR, HRV, and awake-rest SpO₂); enough clean successive intervals for HRV; ≥ 8 valid 30-minute SpO₂ slots (4 hours) for that SpO₂ series; a stored step/activity stream for steps and active minutes. Failed days are missing or low_quality, not zero. Store quality_status and quality_reason (condensed 1.7). SpO₂ slots that are missing, non-finite, or outside 70–100% are dropped before the daily mean or nadir.

7-day coverage = n_7 / 7 (need 4 to publish). Missing days are omitted from the EWMA and the remaining weights are renormalized.
Longer coverage = n_long / 53.

Motion on the companion device is the same step/activity stream: still vs moving as the rest-series gate, plus daily waking steps and daily active minutes as two waking_load series. It is not an always-on 100 Hz recording.

NightSignal uses HR when still. Open Wearables: sleep-window filter for variability only, not their 7-day CV or SDNN fallback.

4.6 Context

sleep, awake_rest, awake_active, continuous, or waking_load, copied onto both snapshots. Never mix nights, still-awake, moving-awake, and all-hours copies. Never mix SpO₂ mean and SpO₂ nadir. Never mix steps and active minutes. Never mix WHOOP RMSSD with Apple SDNN.

4.7 Valid-day count

Count of quality-OK days that entered that copy. Not “days since pairing.”

7-day: 0–7 quality-OK (n_7); n_learn is the sum of Student-t weights. Publish a provisional copy if n_7 ≥ 4. Update center_7 only if n_learn ≥ 4.
Longer: <4 calibrating; 4–13 provisional; ≥14 established. Trimmed count is n_long after S1.

4.8 Last update

Most recent quality-OK day inside that window. 7-day last update can be T−1. Longer last update cannot be newer than T−8. Scoring a bad day does not move the date. Device-wide stale: no quality-OK day anywhere for more than 14 days.

4.9 Version

7-day (primary): ewma-span7-mad-k2-7-t4 plus epoch id and param_set = v1.
Longer (secondary): gap7-med-mad-k2-60-cp1 plus the same epoch id and param_set.

Late-arriving WHOOP 5.0 history writes a new snapshot version. It does not edit old ones. If a bounded EWMA is added later on the gapped 53-day list, that is a new longer version string. It is not this 7-day EWMA. Changing α, k_band, floors, ν, k_cusum, h_cusum, p_thr, or n_borrow is a new param_set, not an edit.

4.10 Fast–slow gap

gap = center_7_raw − center_long
gap_z = gap / spread_long
Stored every day both copies exist. The later watchdog reads this before 7-day z.

4.11 Confidence percentage and alert_eligible

confidence_pct_7 and confidence_pct_long as in condensed section 1.7 (valid days × coverage × signal-quality fraction × stability). Also store show_7, show_long, established_long, conf_spread, alert_eligible. Four 7-day points can show. They cannot set alert_eligible by themselves. The percentage is not Charge.

Every non-OK day stores quality_status and quality_reason (condensed 1.7). Low quality is not trained and is not written as 0.

4.12 Learn mask

Per day in [T−7, T−1]: λ(d) ∈ (0, 1] from Student-t (ν = 4) or Huber. n_learn = sum λ(d). Days with small λ remain in n_7 and in center_7_raw.

4.13 Persistence

run_length, worse, hits_3, two_of_three on z_long. Not an alert. NightSignal-style successive nights are these fields, stored now, paged later.

4.14 Regime

regime_shift when p_change ≥ p_thr; also store S_t and p_change. Held-or-live flag for center_long. A qualifying start opens a new adaptive epoch for Usual since treatment and freezes Usual before treatment (section 1.8). Dose change is not a new epoch.

4.15 Layer

layer = 1 (this series only). Layer 2 combinations are out of this snapshot.

4.16 Parameters

param_set = v1 and the list in section 1.7. Validate on real WHOOP 5.0 days before treating any constant as final.

4.17 Mode

mode = adaptive (always). mode = trial on the frozen bundle after a qualifying t0. Both may exist on the same T. They are not averaged.

4.18 Trial freeze

trial_freeze_ok, t0, T_freeze, trial_id, event type, phase, wash_in_days, washout_days. Frozen copies of 4.1–4.16 plus slope_long, baseline period, σ_meas, MDC_95, r1, n_eff. Immutable after write.

4.19 Qualification

Per-metric gates in section 1.8. waking_load needs n_long ≥ 21. Rest series need n_long ≥ 14. SpO₂ needs slot coverage on ≥ 10 long nights. Fail → no trial z.

4.20 Trajectory contrast

expected(T), delta_trial_level, z_trial_level, delta_trial_traj, z_trial_traj, above_mdc. Primary trial number is z_trial_traj (physiological change since intervention, not a causal drug effect).

4.21 Missingness and confounders

missing_cause and confounder flags on the daily observation. Primary trial contrast skips days that are not (none, none). Do not recode missing as zero.

4.22 Provenance

device_model, firmware, decoder_version, metric_def_version, sensor_source, raw_ref. Frozen at t0. provenance_break if any field changes after t0.


5. How to build it

Same eight steps as the condensed plan.


Step 1. Types
Series key: person, metric, WHOOP 5.0, derivation, context (sleep, awake_rest, awake_active, continuous, or waking_load), epoch.
Daily observation: calendar day, value, coverage minutes or 30-minute slot count, quality flag, missing_cause, confounder flags, treatment phase.
Snapshot: every field in section 2, twice (longer median first, then 7-day EWMA), plus gap, persistence, confidence, regime_shift, param_set, provenance. After a qualifying t0, a frozen trial bundle (section 1.8) sits beside the live adaptive snapshot.
Versions: ewma-span7-mad-k2-7-t4 (primary) and gap7-med-mad-k2-60-cp1 (secondary), plus epoch id and param_set = v1. Trial bundle copies those strings at freeze and never changes them.
Intervention event: timestamp, type (start, dose_change, interruption, restart, stop), trial_id, dose fields.
...
Longer: trim, median, MAD, floor, ±k band, delta, z, regime check.
7-day: Student-t downweight vs established long, α from span_7, renormalize over w_learn, MAD around that EWMA, borrow spread, floor, ±k band, delta, z, gap, persistence.
...
A start event that qualifies → frozen bundle written; a later 61 bpm day may move adaptive center_7 and must not change center_long_frozen.
expected(T) uses frozen slope; today vs frozen mean alone is not the only trial contrast.
...
Step 7. Shadow log
Write the adaptive snapshot, including gap, confidence, persistence, regime_shift, missing_cause, confounders, and provenance. If a qualifying freeze exists, write trial deltas beside it. Do not feed Charge. Do not add alerts or a weighted 0–100 mix. Do not let post-t0 days edit the frozen bundle.

Step 8. After this ships
A later watchdog can read z_long, gap, two_of_three, run_length, worse, alert_eligible on the adaptive track, then (later still) combinations across series. Trial z_trial_traj is for physiological change since intervention, not for that watchdog’s “are they off usual now” page. An optional bounded EWMA on the gapped 53-day list is not the returned longer center. Parameter values are v1 until a data pass replaces param_set.


6. Worked math

Lists are oldest → newest. Longer copy first (trim). 7-day center is the span-7 EWMA on learn days (α from span_7, renormalized). Spread is 1.4826 × MAD around that EWMA, borrowed from the longer spread when n_learn < 7, with the floor. param_set = v1.

6.1 Sleep resting heart rate (floor 2 bpm)
...
7-day band = 61.1 ± 5.70 = 56.4 to 66.8 bpm
...
If those seven days were all 72 and the longer usual is 60 (established, spread_long = 2.85), each λ ≈ 0.23 so n_learn ≈ 1.6 < 4 and center_7 is held at 61.1; center_7_raw = 72; gap = 12 bpm; gap_z = 12 / 2.85 = 4.2; z_long(T) if today is also 72 is 4.2. The 7-day training center does not walk up. See 6.18–6.19.

6.2 Awake-rest heart rate (floor 3 bpm; need ≥30 still minutes that day)
...
6.3 Sleep HRV (RMSSD; math on ln; floor 0.08 on the log scale)
...
6.4 Awake-rest HRV (same recipe; only if still-waking coverage is enough)
...
7-day band in ms = 26.6 to 36.3 ms

Today RMSSD = 22 ms, ln = 3.091
7-day delta = 22 − 30.1 = −8.1 ms
7-day z = (3.091 − 3.403) / 0.08 = −3.9

6.5 Sleep respiration (floor 0.5 /min; only if the nightly rate is stored and validated)
...
6.6 Sleep wrist temperature (floor 0.3 °C)
...
6.7 Sleep SpO₂ mean (floor 0.5 percentage points; 30-minute samples)
...
6.8 Sleep SpO₂ nadir (own series; floor 0.5 percentage points)
...
6.9 Awake-rest SpO₂ mean

Same 30-minute math as 6.7, but only waking-and-still slots, ≥ 8 valid. Same EWMA / MAD / floor on those daily means. Do not fold sleep slots into this series.

6.10 Daily steps (floor 500 steps; waking_load)
...
6.11 Daily active minutes (floor 10 min; waking_load)
...
6.12 Awake-active heart rate (floor 4 bpm; need ≥30 moving minutes that day)
...
6.13 Awake-active HRV (same recipe; only if moving-waking coverage is enough)
...
6.14 Awake-active SpO₂ mean

Same 30-minute math as 6.7, but only waking-and-moving slots, ≥ 8 valid. Do not fold sleep or still-awake slots into this series.
...
6.15 Continuous heart rate (floor 5 bpm; need ≥240 valid minutes that civil day)
...
Same 7-day tape as 6.7: center_7 = 96.43 %, spread = 0.50, today 94 %, z = −4.9.



6.18 Student-t downweight on the 7-day center (sleep RHR; long usual established)
...
6.19 Hold when the whole week is off (sleep RHR)
...
6.20 Persistence fields (still not an alert)
...
6.21 Borrowed spread with four learn days
...
6.22 Trim vs online change-point on the longer copy
...
6.23 Two tracks after treatment start (sleep RHR; same EWMA/MAD math)

Qualifying freeze at t0. Frozen bundle: center_long = 68 bpm, spread_long = 2.85 bpm, slope_long = +0.05 bpm/day, n_long = 40, trial_freeze_ok = 1, phase clock wash_in_days = 7.
T_freeze is the last completed day before t0.

28 days after T_freeze, today = 61 bpm, confounder = none, missing_cause = none, phase = steady_state.
Adaptive track has been allowed to learn: center_7 ≈ 61.2 bpm. Monitoring z vs that center is ~0.

expected(T) = 68 + 0.05 × 28 = 69.4 bpm
delta_trial_level = 61 − 68 = −7.0 bpm
z_trial_level = −7.0 / 2.85 = −2.5
delta_trial_traj = 61 − 69.4 = −8.4 bpm
z_trial_traj = −8.4 / 2.85 = −2.9

center_long_frozen stays 68. The 61s do not enter it. σ_meas = 1.0 bpm → MDC_95 = 2.77 bpm. |−8.4| > 2.77 → above_mdc = 1.

6.24 Qualification miss

start event, sleep RHR n_long = 8. trial_freeze_ok = 0. No frozen bundle. Adaptive snapshots continue. Do not write a trial z.

6.25 Missingness and provenance
...
Cadence — 14-day honesty before an established usual; keep RMSSD and SDNN in separate series. Four 7-day points may show; they do not make alert_eligible.
https://github.com/rajanshxrma/cadence

Vela — median, 1.4826 × MAD, ln(HRV) z-score, missing stays missing, as-of cutoff
...
Gadaleta et al., npj Digital Medicine, 2021 — last 7 days left out of the longer median (those 7 days are the primary EWMA)
...
HRV-Foundation-Forecasting — EWMA as a classical short-horizon tracker (7-day center).
